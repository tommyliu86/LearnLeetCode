   ...
   static class Solution {
        public int minimizeArrayValue(int[] nums) {
            ...
            for (int i = 1; i < maxs.length; i++) {
                ...
                if (nums[i] > maxs[i - 1]) {
                    int j =(int)( sums[i] / (i + 1));
                    int k=(int) (sums[i]%(i+1));
                    if (j < maxs[i - 1]) {
                        maxs[i] = maxs[i - 1];
                    } else {
                        maxs[i] = j+(k>0?1:0);
    ...
    class Solution4 {
        ...
        private int qua(int[] powers, int start, int end) {
            long ans = 1;
    ...
    static class Solution3 {
        ...
        public int[] productQueries(int n, int[][] queries) {
            ...
            long[] pows = new long[powers.length + 1];
            pows[0] = 1;
            int[] count = new int[pows.length];
            int cc = 0;
            for (int i = 1; i < pows.length; i++) {
                pows[i] = ((long) pows[i - 1] * powers[i - 1]);
                if (pows[i] > mod) {
                    pows[i] = pows[i] % mod;
                    count[i] = ++cc;
                }
            ...
            for (int i = 0; i < ans.length; i++) {
                ...
                int start = query[0];
                int end = query[1];
                long pow1 = pows[start];
                long pow2 = pows[end + 1];

                ans[i] = (int) (((pow2 + count[end + 1] * mod) / (pow1 + count[start] * mod)));
